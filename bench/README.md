...
Does splitting an agent into a decision model (Jev) and a small local executor beat a conventional
single-model coding agent running on the same local model? This harness answers that with numbers:
...
| `--verify` | — | score each task's seed (must fail) and reference solution (must pass); no agents run |
...
- **OpenCode gets a throwaway config home** (`XDG_CONFIG_HOME`, `XDG_DATA_HOME`, `XDG_STATE_HOME`), with
  ...
  into the folder the benchmark was started from. The harness now warns if any new file appears there.
