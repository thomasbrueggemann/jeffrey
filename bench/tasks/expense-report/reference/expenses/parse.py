...
def parse_line(fields: list[str]) -> Expense:
    ...
    if not amount.is_finite():
        raise ValueError(f"bad amount {raw_amount!r}")
    if amount < 0:
        raise ValueError(f"negative amount {raw_amount!r}: refunds are not supported")
    if amount.as_tuple().exponent < -2:
        raise ValueError(f"amount {raw_amount!r} has more than two decimal places")
